...
app.get("/receive", function (req, res) {
    res.sendFile(__dirname + "/views/receive.html");
});
...
app.post("/login", function (req, res) {
    ...
    if (Acc.checkUsername(email) == false) {
        if (Acc.checkLogin(email, password) == false) {
            res.render("loginError");
        }
    } else {
        (Acc.checkLogin(email, password) == true)
        res.render("loginSuccess", {"username": email, "password": password});
